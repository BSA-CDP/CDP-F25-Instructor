...
void setup() {
  ...
  Serial.println("Feather RP2040 - SD Test");
  ...
  // File Management
  if (SD.exists("SD_Test_BMP388_data.csv")) {   // Check if a data file already exists
    SD.remove("SD_Test_BMP388_data.csv");       // Delete it to start fresh
    Serial.println("Existing data file deleted.");
  }

  dataFile = SD.open("SD_Test_BMP388_data.csv", FILE_WRITE);  // Open new file for writing
  if (dataFile) {
    ...
    Serial.println("Error opening SD_Test_BMP388_data.csv");  // Print message if file couldn't be created
...
void loop() {
 ...
 // Log data to the file if the SD file can be found
  if (sdAvailable()) {
    ...
    dataFile = SD.open("SD_Test_BMP388_data.csv", FILE_WRITE); // Reopen file in prep to write to it
